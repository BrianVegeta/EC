class Ajax::Api::ItemController < ApplicationController
  ...
  def space_params
   ...
   def search_item_params
     ...
     # pricehigh : Int => 最高價格
     # pricelow : Int => 最低價格
     # starttime : Long => 搜尋建立時間(開始)
     # endtime : Long => 搜尋建立時間(結束)
     # sorts : List<Object> => 篩選條件
    
     params.permit(:city, :area, :pricehigh, :pricelow, :starttime, :endtime, :sorts).merge(paging_params)
