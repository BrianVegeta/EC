...
= userprofile =
===============================================>>>>>*/
const ACTION_PREFIX = 'MY.COMMENT';
const REDUCER_KEY = 'myComment';
const SIZE = 9;
...
export function fetchRecords(target, recursiveRecords = []) {
  return (dispatch, getState) => {
    const { auth } = getState();
    ...
    .then((data) => {
      ...
      if (reducedRecords.length < data.length && recursiveTimes <= RECURSIVE_LIMIT) {
        /* RECURSIVE AGAIN */
        dispatch(fetchRecords(currentUser.uid, target, reducedRecords));
        return;
      }
